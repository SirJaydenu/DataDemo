...
def get_monthly_averages(data_list):
    FILE_NAME = "table.csv"
    list = get_data_list(FILE_NAME)
    file = open(FILE_NAME, "r")
    line = file.read()
    data_list = list
    for x in range(1):
        data_list.append(print(line.split(",")))
        print()
    return data_list
...
def print_info(monthly_average_list):
	print("Top 6 best months for google stocks: ")
	print("Top 6 worst months for google stocks: ")
	
	return
#get_data_list("table.csv")
get_monthly_averages("table.csv")
print("f")
